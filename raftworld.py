...
if __name__ == "__main__":
    ...
    if token:
        ...
        print(f"🔍 С 20 по 40 символ: {token[20:40] if len(token) > 40 else 'хуй'}")
        print(f"🔍 Есть ли дефис в конце? {'Да' if token.endswith('-') else 'Нет'}")
        print(f"🔍 Есть ли подчеркивание в конце? {'Да' if token.endswith('_') else 'Нет'}")
        print(f"🔍 Предпоследний символ: {token[-2] if len(token) > 1 else 'хуй'}")
        print(f"🔍 Последний символ: {token[-1]}")
